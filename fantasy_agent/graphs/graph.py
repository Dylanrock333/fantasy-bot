...
def _personality_system() -> SystemMessage:
    return SystemMessage(content=(
        ...
        "Keep replies SHORT by default: roughly 1-6 sentences, using your "
        "judgment on where in that range to land based on how much "
        "relevant info there actually is to cover - a one-line answer "
        "doesn't need padding up to 6, and a genuinely multi-part question "
        "can use the full range. Never more than "
        "a tight bulleted list for things like standings or rosters, "
        "unless the user's prompt explicitly asks for a long/detailed/"
        "in-depth response - only then should you go longer than the "
        "default range. No "
